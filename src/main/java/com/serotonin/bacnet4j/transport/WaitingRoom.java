...
public class WaitingRoom {
    static final Logger LOG = LoggerFactory.getLogger(WaitingRoom.class);
    //    static {
    ...
    public WaitingRoomKey enterServer(Address address, OctetString linkService, byte id) {
        ...
        synchronized (waitHere) {
            ...
            if (LOG.isTraceEnabled())
                LOG.trace("WaitingRoom.enterServer: key=" + key);
        }
    ...
    public void leave(WaitingRoomKey key) {
        synchronized (waitHere) {
            waitHere.remove(key);
            if (LOG.isTraceEnabled())
                LOG.trace("WaitingRoom.leave: key=" + key);
        }
    }
    ...
    public void notifyMember(Address address, OctetString linkService, byte id, boolean isFromServer, APDU apdu) {
        ...
        synchronized (waitHere) {
            LOG.warn("WaitingRoom.notifyMember: no waiting recipient for message: key=" + key + ", message=" + apdu
                    + ", waiting room=" + waitHere);
        }
    }
